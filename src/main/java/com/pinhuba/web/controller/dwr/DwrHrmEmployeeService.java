...
public class DwrHrmEmployeeService {
	...
	public ResultBean listEmployees(ServletContext context, HttpServletRequest request, HrmEmployee employee, Pager pager) {
		...
		if(employee.getGroupId() > 0){
			pager = PagerHelper.getPager(pager,employeeinfoService.listEmployeeByGroupIdCount(employee));
			list = employeeinfoService.listEmployeeByGroupId(employee, pager);
		}else{
			pager = PagerHelper.getPager(pager,employeeinfoService.listEmployeeCount(employee));
			list = employeeinfoService.getAllEmployee(employee, pager);
		}
	...
	public List<HrmDepartment> listDownDepartByCode(HttpServletRequest request,String deptCode){
		List<HrmDepartment> deptList = employeeinfoService.getDownDeptartByCode(deptCode);
	...
	public int listDownDepartByCodeCount(HttpServletRequest request,String deptCode){
		return employeeinfoService.getDownDeptartByCodeCount(deptCode);
	}
	/**********************************************
	...
	public ResultBean saveEmployee(ServletContext context, HttpServletRequest request, HrmEmployee employee, String attach, String imgfile) {
		...
		if (employee.getPrimaryKey() != null && employee.getPrimaryKey().length() > 0) {  //编辑
			...
			employee.setPrimaryKey(UtilPrimaryKey.getPrimaryKey(""));
		}
	...
	public ResultBean setSeparationByIds(ServletContext context, HttpServletRequest request, String[] employeePKs) {
	    employeeinfoService.setSeparationByPks(employeePKs);	
		logger.info("设置离职人员...");
		return WebUtilWork.WebResultPack(null);
	}
	...
	public ResultBean listDepartments(ServletContext context, HttpServletRequest request, HrmDepartment department, Pager pager) {
		...
		try {
		    ...
		    pager = PagerHelper.getPager(pager,employeeinfoService.listDepartmentCount(department));
		    list = employeeinfoService.getAllDepartment(department, pager);
		    ...
		    for(HrmDepartment oaDepartment : list){
				...
				if(oaDepartment.getHrmDepUpid() != null && oaDepartment.getHrmDepUpid().length() > 0){
					HrmDepartment updep = employeeinfoService.getDepartmentByCode(oaDepartment.getHrmDepUpid());
	...
	public ResultBean treeMoveShowRow(HttpServletRequest request,long pk,int type){
		...
		List<HrmDepartment> deptList = employeeinfoService.getDownDeptartByCode(dept.getHrmDepUpid());
	...
	public ResultBean treeMoveShowRowByPost(HttpServletRequest request,long pk,int type){
		HrmPost post = employeeinfoService.getPostByPk(pk);
		List<HrmPost> postList = employeeinfoService.getHrmPostListByUpcode(post.getHrmPostUpid());
	 ...
	 * @return
	 */
	public ResultBean getAlldepartmentNopager(ServletContext context, HttpServletRequest request){
		List<HrmDepartment> list = new ArrayList<HrmDepartment>();
		list = employeeinfoService.getAllDepNopager();
		return WebUtilWork.WebResultPack(list);
	}
	...
	public ResultBean saveDepartment(ServletContext context, HttpServletRequest request,HrmDepartment department) {
		...
		if(upPk != null && upPk.length()>0){
			...
			count = employeeinfoService.getDepartmentCountBynameAndCode(department.getHrmDepName(),tmpUpCode,0);
		}else{
			count = employeeinfoService.getDepartmentCountBynameAndCode(department.getHrmDepName(),tmpUpCode,0);
		}
		if(count>0){
	...
	public ResultBean deleteDepartmentById(ServletContext context,HttpServletRequest request,long[] ids){
		for (long l : ids) {
			...
			if (dept.getHrmDepId()!=null&&dept.getHrmDepId().length()>0) {
				int row = employeeinfoService.getdepartmentByUpCode(dept.getHrmDepId());
				if (row>0) {
			...
			int empcount = employeeinfoService.listEmployeeCount(employee);
			if (empcount>0) {
	...
	public ResultBean getDepartmentByPK(ServletContext context, HttpServletRequest request, long departmentPK) {
		...
		//加载上级部门
		if(department.getHrmDepUpid() != null && department.getHrmDepUpid().length() > 0){
			HrmDepartment updep = employeeinfoService.getDepartmentByCode(department.getHrmDepUpid());
	...
	public ResultBean updateDepartment(ServletContext context, HttpServletRequest request,HrmDepartment department,String oldDepUpId) {
		...
		if(department.getHrmDepUpid() != null && department.getHrmDepUpid().length()>0){
			...
			// 存在上级部门
			tmpUpCode = hDept.getHrmDepId();
			count = employeeinfoService.getDepartmentCountBynameAndCode(department.getHrmDepName(),tmpUpCode,department.getPrimaryKey());
		}else{
			count = employeeinfoService.getDepartmentCountBynameAndCode(department.getHrmDepName(),tmpUpCode,department.getPrimaryKey());
		}
		if(count>0){
		...
		if(tmpUpCode.equals(oldDepUpId)){//上级编码未变动
			...
			boolean bl = false;
			//判断上级编码是否为自己和自己的下级
			List<HrmDepartment> tmpList = employeeinfoService.getHrmDepartmetnListByDepId(dept.getHrmDepId());
		...
		//更新下级部门编码
		if(newDepId!=null){
			employeeinfoService.updateBatchHrmDepartmentIdAndUpId(dept.getHrmDepId(), newDepId);
		}else{
	...
	public ResultBean listPostByPager(ServletContext context, HttpServletRequest request, HrmPost hrmPost, Pager pager) {
		pager = PagerHelper.getPager(pager,employeeinfoService.listPostCount(hrmPost));
		List<HrmPost>  list = employeeinfoService.getAllPost(hrmPost,pager);
		return WebUtilWork.WebResultPack(list, pager);
	}
	...
	public ResultBean savePost(ServletContext context, HttpServletRequest request, HrmPost post) {
		...
		if (post.getPrimaryKey() > 0) {   //编辑
			tmp = employeeinfoService.getPostByPk(post.getPrimaryKey());
			if(post.getHrmPostUpid().equals(tmp.getHrmPostUpid())){//上级编码未变动
				...
				boolean bl = false;
				//判断上级编码是否为自己和自己的下级
				List<HrmPost> tmpList = employeeinfoService.getHrmPostListByPostId(tmp.getHrmPostId());
		...
		if(newPostId!=null&&tmp!=null){
			employeeinfoService.updateBatchHrmPostIdAndUpId(tmp.getHrmPostId(), newPostId);
		}else{
			logger.info("编码未变动不用更新");
		}
		return WebUtilWork.WebResultPack(null);
	...
	public ResultBean updatePostManger(ServletContext context, HttpServletRequest request,long postid,String empid){
		...
		String nowtime = UtilWork.getNowTime();
		HrmPost tmp = employeeinfoService.getPostByPk(postid);
	...
	public ResultBean getPostByPk(ServletContext context, HttpServletRequest request, long postPk) {
		HrmPost post = employeeinfoService.getPostByPk(postPk);
	...
	public ResultBean deletePostsByPks(ServletContext context, HttpServletRequest request, long[] postPks) {
		for (long l : postPks) {
			HrmPost post = employeeinfoService.getPostByPk(l);
			int count = employeeinfoService.getHrmPostByUpcodeCount(post.getHrmPostId());
			if (count>0) {
				return new ResultBean(false,post.getHrmPostName()+" 岗位有下级岗位 "+count+" 个,不能删除！");
			}
			int row = employeeinfoService.getEmployeeCountByPostId(l);
			if (row>0) {
	...
	public List<HrmPost> getHrmPostByUpCode(HttpServletRequest request,String upcode){
		return employeeinfoService.getHrmPostListByUpcode(upcode);
	}
	...
	public int getHrmPostByupcodeCount(HttpServletRequest request,String upcode){
		return employeeinfoService.getHrmPostByUpcodeCount(upcode);
	}
	...
	public ResultBean getHrmEmployeeListByMainPostId(ServletContext context,HttpServletRequest request,int postid,Pager pager){
		pager= PagerHelper.getPager(pager, employeeinfoService.getEmployeeCountByMainPostId(postid));
		List<HrmEmployee> employeeList = employeeinfoService.getEmployeeeByMainPostId(postid,pager);
