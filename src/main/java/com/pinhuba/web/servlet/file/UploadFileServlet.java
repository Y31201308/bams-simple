...
public class UploadFileServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 符合要求开始上传
		String saveDirectory = SystemConfig.getParam("bams.upload.imgSavePath");
		if (savetype.equalsIgnoreCase("file")) {
			saveDirectory = SystemConfig.getParam("bams.upload.fileSavePath");
		}
		...
		if (savedir.length() <= 0) {
			saveDirectory += UtilTool.getUserPath(request, false) + ConstWords.septor;
		}
		...
		try {
			...
			if (forward.equalsIgnoreCase("img") && oldimg != null && oldimg.length() > 0) {
				...
				String back = "var tmpv = document.getElementById('tmp').value;\nwindow.location.href = '" + request.getContextPath() + "/bams/imageupload.jsp?defaultImg='+tmpv+'&he=" + he + "&AcceptText=" + aname + "&edit=true';";
		...
		if (forward.equalsIgnoreCase("img")) {
			url = "bams/imageupload.jsp";
		} else if (forward.equalsIgnoreCase("file")) {
			url = "bams/fileupload.jsp";
		}
