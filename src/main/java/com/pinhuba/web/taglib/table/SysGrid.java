...
public class SysGrid {
	...
	private void setParam(HttpServletRequest request){
		this.tableRowSize = Integer.parseInt(UtilTool.getSysParamByIndex(request, "bams.grid.pageSize"));
		this.isShowImageDetail = UtilTool.getSysParamByIndex(request, "bams.grid.imgViewCk").equals("是");
	}
	...
	private String createProcessTitle() {
		...
		for (int i = 0; i < helpList.size(); i++) {
			...
			if (tb.getImgTilte() != null && tb.getImgTilte().length() > 0) {
				title.append("<td align='center' valign='middle' style='padding-left:10px;width:15px;'>" + tb.getImgTilte() + "</td>\n");
			} else {
	...
	private String createQuerySelect() {
		...
		cols.append("查询&nbsp;&nbsp;<select style =\"width:auto;top:0px;left:0px;\" id='simpleQueryParam' >");
		for (int i = 0; i < columnList.size(); i++) {
			SysGridColumnBean bc = columnList.get(i);
			if (bc.isShowQuerySelsect()) {
