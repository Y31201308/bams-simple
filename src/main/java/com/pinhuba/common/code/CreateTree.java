...
public class CreateTree {
	...
	public static String getTreeSelectXmlPageString(String dwrName,String pojoClass,String folderName,String pojoShortName) {
		...
		sb.append("        if(row>0){\n");
		sb.append("            tmp =\"src=\\\"\"+request.getContextPath()+\"/bams/" + folderName + "/" + pojoShortName + "_select_xml.jsp?code=\"+" + lowerPojo + ".getCode()+\"&treetype=\"+treetype+\"\\\"\";\n");
		sb.append("        }\n");
		sb.append("        //输出树节点\n");
	...
	public static String getTreePageString(String dwrName,String pojoClass,String folderName,String pojoShortName,String pojoName) {
		...
		sb.append("var tree = new WebFXLoadTree(\""+pojoName+"树\",\"<%=contextPath%>/bams/" + folderName + "/" + pojoShortName + "_tree_xml.jsp?code=00<%=tmp%>\",\"treeclick('');\");\n");
		sb.append("</script>\n");
	...
	public static String getTreeXmlPageString(String dwrName,String pojoClass,String folderName,String pojoShortName) {
		...
		sb.append("        if(ischeck != null && ischeck.length() > 0){\n");
		sb.append("            if(row>0){\n");
		sb.append("                tmp =\"src=\\\"\"+request.getContextPath()+\"/bams/" + folderName + "/" + pojoShortName + "_tree_xml.jsp?code=\"+" + lowerPojo + ".getCode()+\"&ischeck=true\\\"\";\n");
		sb.append("            }\n");
		sb.append("            chstr =\"onchange=\\\"getCheckedIds();\\\"  type=\\\"check\\\"\";\n");
		sb.append("            val = " + lowerPojo + ".getPrimaryKey();\n");
		sb.append("        }else{\n");
		sb.append("            if(row>0){\n");
		sb.append("                tmp =\"src=\\\"\"+request.getContextPath()+\"/bams/" + folderName + "/" + pojoShortName + "_tree_xml.jsp?code=\"+" + lowerPojo + ".getCode()+\"\\\"  action=\\\"treeclick('\"+" + lowerPojo + ".getPrimaryKey()+\"','\"+" + lowerPojo + ".getName()+\"');\\\"\";\n");
		sb.append("            }else{\n");
