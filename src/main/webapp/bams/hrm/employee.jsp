...
function update(employeepk){
	openMDITab("<%=contextPath%>/bams/hrm/employee_add.jsp?employeepk="+employeepk+"&tab="+getMDITab());
}
...
function print(){
	if(getAllRecordArray() != false){	
		...
		openMDITab('<%=contextPath%>/bams/report_show.jsp?raq=employee.raq&macrozgh=\\\'' + getAllRecordArray()[0] + '\\\'');
	} else {
		alertmsg("请选择要打印的人员...");
	}
}
