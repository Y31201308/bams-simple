...
if(obj==null){
	...
	fileSize =request.getParameter("fileSize")==null?SystemConfig.getParam("bams.upload.fileSize"):request.getParameter("fileSize");
...
<%}else{%>
	function repSend(){
		...
		if(cf){
			...
			$.get("<%=request.getContextPath()%>/bams/fileProcess.jsp?filepath="+delPaths,
		    function(bl){
	...
	function deleteOkFile(a){
		...
		if(cf){
			...
			$.get("<%=request.getContextPath()%>/bams/fileProcess.jsp?filepath="+tmp[1],
		    function(bl){
