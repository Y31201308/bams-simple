	...
	if(postlist!=null&& postlist.size()>0){
		...
		for(int i=0;i<postlist.size();i++){
			...
			if(treetype==null){
				...
				if(row>0){
					tmp3 ="src=\""+request.getContextPath()+"/bams/tree/posttree_xml.jsp?fid="+post.getHrmPostId()+tmp2+"\" action=\"posttreeclick('"+tmp5+"','"+post.getHrmPostName()+"');\"";
				}else{
					tmp3 ="action=\"posttreeclick('"+tmp5+"','"+post.getHrmPostName()+"');\"";
				}
			}else{
				if(row>0){
					tmp3 ="src=\""+request.getContextPath()+"/bams/tree/posttree_xml.jsp?fid="+post.getHrmPostId()+tmp2+"\"";
				}
