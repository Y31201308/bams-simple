...
function dblCallback(obj){
	var box = new Sys.msgbox('用户权限明细信息','<%=contextPath %>/bams/system_set/premmset_detail.jsp?uid='+obj.value,800,500);
	box.msgtitle='用户附加权限明细信息(不包含角色权限)';
	box.show();
}

function showall(id){
	var box = new Sys.msgbox('用户权限明细信息','<%=contextPath %>/bams/system_set/premmset_detail.jsp?show=all&uid='+id,800,500);
...
function setbatchok(){
	Sys.load('<%=contextPath%>/bams/system_set/premmset_info.jsp?uids='+getAllRecordArray());
}


function set(id){
	var url = '<%=contextPath%>/bams/system_set/premmset_info.jsp?uids='+id;
	url += "&tab="+getMDITab();
	openMDITab(url);
}
