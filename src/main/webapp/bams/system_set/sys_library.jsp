...
function edit(id){
	openMDITab("<%=contextPath%>/bams/system_set/sys_library_add.jsp?lid="+id+"&tab="+getMDITab());
}

function add(){
	openMDITab("<%=contextPath%>/bams/system_set/sys_library_add.jsp?tab="+getMDITab());
}
...
function setvalid(pk,sta){
	dwrSysProcessService.setSysLibraryInfoBypk(pk,sta,setcallback);
}
...
function setbatch(){
	...
	if( pks!= false){
		dwrSysProcessService.setSysLibraryInfoByPks(pks,setcallback);
	}else{
		alertmsg("请选择要设置的记录...");
	}
...
btnList.add(new SysGridBtnBean("有效性","setbatch()","valid.png"));
